...
**图片整理 (PhotoClassifier)** 是一款 macOS 原生桌面应用，用于快速整理和归类照片/视频文件。核心理念是"目录即标签"——通过将文件移动到不同子目录来实现归类，无需数据库，归类结果直接体现在文件系统中。
...
1. **无需数据库** — 归类结果直接体现在文件系统中，关闭应用不丢失
2. **跨工具兼容** — 用 Finder 也能看到归类结果
